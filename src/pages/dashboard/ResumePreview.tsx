...
/** parse "32%" -> 32 */
function pctToNumber(v: any, fallback: number) {
...
/** parse "24px" -> 24 */
// function pxToNumber(v: any, fallback: number) {
//   if (typeof v === "number") return v;
//   if (typeof v !== "string") return fallback;
...
function paperStyle(theme: any): React.CSSProperties {
  ...
  // ✅ now supports pagePadding from theme (number in px)
  const padding = theme?.pagePaddingPx ?? theme?.cardPadding ?? 32;
...
function buildDummyData(_schema: any) {
  return {
    ...
    projects: [
      { name: "Resume Builder", desc: "Schema-driven templates + exports." },
    ],
    ...
    volunteering: ["Community mentor", "Open-source contributions"],
  };
}
...
function HeaderBlock({ theme, data }: any) {
  ...
  if (hv === "rolePill") {
    // ✅ role badge colors if provided
    const badgeBg = theme?.roleBadgeBg || primary;
    const badgeText = theme?.roleBadgeText || "#111827";
    return (
...
function AvatarBlock({ theme, data }: any) {
  ...
  // ✅ If no image, still show placeholder (so templates with avatar look different)
  const showPlaceholder = theme?.avatarPlaceholder !== false;
...
export default function ResumePreview({
        ...
        {pdfPages.length === 0 ? (
          <div style={{ ...paperStyle(theme), ...bodyStyle(theme), padding: 22 }}>
            ...
            <div style={{ marginTop: 8, fontSize: 12, color: muted(theme) }}>
              PDF pages not found in schema/template.
            </div>
          </div>
        ) : (
  ...
  const renderList = (id: string, items: any[]) => (
    <Block id={id}>
      <div style={{ display: "grid", gap: 6 }}>
        {(items || []).slice(0, 8).map((x: any, idx: number) => (
  ...
  const renderTimeline = (id: string, items: any[]) => (
    <Block id={id}>
      {(items || []).slice(0, 4).map((x: any, idx: number) => (
        <div key={idx} style={{ marginBottom: 14, pageBreakInside: "avoid" }}>
          ...
          {Array.isArray(x.bullets) ? (
            <ul style={{ margin: "8px 0 0", paddingLeft: 18, fontSize: 10 }}>
              {x.bullets.slice(0, 4).map((b: string, i: number) => (
  ...
  /** ✅ NEW: support column widths + left panel background for Two Column */
  const leftW = pctToNumber(theme?.leftWidthPct ?? theme?.leftWidth ?? theme?.left_width, 40);
  const rightW = pctToNumber(theme?.rightWidthPct ?? theme?.rightWidth ?? theme?.right_width, 60);

  const leftPaneBg = theme?.leftPaneBg || theme?.sidebarBg; // external two_column can use this
  const leftPaneText = theme?.leftPaneText || theme?.sidebarText || theme?.sidebarTextColor;
  const leftPaneAccent = theme?.leftPaneAccent || theme?.sidebarAccent || theme?.primary;

  const TwoColLeftBox: React.FC<{ children: any }> = ({ children }) => (
    <div
      style={{
        width: `${leftW}%`,
        background: leftPaneBg || "transparent",
        color: leftPaneText || "#111827",
        padding: leftPaneBg ? 18 : 0,
        borderRadius: leftPaneBg ? 14 : 0,
      }}
    >
      {leftPaneBg ? (
        <div
          style={{
            height: 8,
            borderBottom: `3px solid ${leftPaneAccent || "#2563eb"}`,
            marginBottom: 10,
            borderRadius: 999,
          }}
        />
  ...
  const TwoColRightBox: React.FC<{ children: any }> = ({ children }) => (
    <div style={{ width: `${rightW}%` }}>{children}</div>
  );
  ...
  const SidebarBox: React.FC<{ children: any }> = ({ children }) => (
    ...
    >
      <div
        style={{
          height: 8,
          borderBottom: `3px solid ${sidebarAccent}`,
          marginBottom: 10,
          borderRadius: 999,
        }}
      />
  ...
  const MainBox: React.FC<{ children: any }> = ({ children }) => (
